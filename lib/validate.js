...
const validateSimpleProperty = async (
  ...
  let validators = []
  // If validators is an array, use those, regardless of `set`
  // This is the shorthand for `default`
  if (Array.isArray(property.validators)) {
    validators = property.validators
  } else if (property.validators) {
    // If targeting a specific set and specific set exists,
    // validate with that set
    if (set !== 'default' && property.validators[set]) {
      validators = property.validators[set]
    } else if (property.validators.default) {
      // Otherwise, if not targeting a specific set (or specific set doesn't exist)
      // fallback to using `default`
      validators = property.validators.default
    }
    // Always include `all` validation set if it exists
    if (property.validators.all) {
      validators = Array.from(property.validators.all).concat(validators)
    }
  }
  const errorName =
  ...
  if (validators === undefined || !Array.isArray(validators)) {
    return false
  }
  ...
  async (...args) => {
    ...
    if (typeof callback === 'function') {
      validateRecursive(internalSchema)(entityObject, set, tag)
        // These have been "made safe" using setTimeout, but the rule can't tell
        //eslint-disable-next-line promise/no-callback-in-promise
        .then((response) => setTimeout(() => callback(null, response), 0))
        //eslint-disable-next-line promise/no-callback-in-promise
