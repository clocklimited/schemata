...
const validateSimpleProperty = async (
  ...
  if (Array.isArray(property.validators)) {
    ...
    // validate with that set AND `all`
    if (set !== 'all' && property.validators[set]) {
      validators = property.validators[set].concat(property.validators.all)
    } else {
      // Otherwise, if not targeting a specific set (or specific set doesn't exist)
      // fallback to using `all`
      validators = property.validators.all
    }
  }
  const errorName =
  ...
  if (validators === undefined || !Array.isArray(validators)) return false
  const promises = validators.filter(Boolean).map((validator) => {
